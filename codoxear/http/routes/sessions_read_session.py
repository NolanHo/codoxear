...
def handle_get(runtime: ServerRuntime, handler: Any, path: str, u: Any) -> bool:
    if path.startswith("/api/sessions/") and path.endswith("/live"):
        ...
        try:
            payload = runtime._session_live_payload(
                runtime.MANAGER,
                session_id,
                offset=offset,
    ...
    if path.startswith("/api/sessions/") and path.endswith("/workspace"):
        ...
        if not session_id:
            ...
            return True
        try:
            payload = runtime._session_workspace_payload(runtime.MANAGER, session_id)
        except KeyError:
            runtime._json_response(handler, 404, {"error": "unknown session"})
            return True
        except ValueError as exc:
    ...
    if path.startswith("/api/sessions/") and path.endswith("/details"):
        ...
        if not session_id:
            ...
            return True
        try:
            payload = runtime._session_details_payload(runtime.MANAGER, session_id)
        except KeyError:
            runtime._json_response(handler, 404, {"error": "unknown session"})
            return True
        runtime._json_response(handler, 200, payload)
    ...
    if path.startswith("/api/sessions/") and path.endswith("/diagnostics"):
        ...
        if not s:
            ...
            return True
        try:
            state = runtime._validated_session_state(runtime.MANAGER.get_state(session_id))
        except ValueError as exc:
            runtime._json_response(handler, 502, {"error": str(exc)})
            return True
        ...
        cwd_path = runtime._safe_expanduser(Path(s.cwd))
        if not cwd_path.is_absolute():
            cwd_path = cwd_path.resolve()
        git_branch = runtime._current_git_branch(cwd_path)
        updated_ts = runtime._display_updated_ts(s)
        elapsed_s = max(0.0, time.time() - updated_ts)
        time_priority = runtime._priority_from_elapsed_seconds(elapsed_s)
        base_priority = runtime._clip01(time_priority + float(sidebar_meta["priority_offset"]))
        ...
        broker_busy = runtime._state_busy_value(state)
        busy = runtime._display_pi_busy(s, broker_busy=broker_busy) if s.backend == "pi" else broker_busy
        ...
        runtime._json_response(
            ...
            200,
            {
                "session_id": runtime._durable_session_id_for_live_session(s),
                ...
                "session_file_path": runtime._display_source_path(s),
                ...
                "provider_choice": runtime._provider_choice_for_backend(
                    backend=s.backend,
                    model_provider=model_provider,
                    preferred_auth_method=preferred_auth_method,
                ),
                ...
                "todo_snapshot": runtime._todo_snapshot_payload_for_session(s),
            },
        )
        return True
    ...
    if path.startswith("/api/sessions/") and path.endswith("/queue"):
        ...
        try:
            ...
            runtime._json_response(handler, 502, {"error": str(exc)})
            return True
        runtime._json_response(handler, 200, {"ok": True, "queue": q})
        return True
    ...
    session_id = runtime._match_session_route(path, "commands")
    ...
    if path.startswith("/api/sessions/") and path.endswith("/messages"):
        ...
        s = runtime.MANAGER.get_session(session_id)
        historical_row = runtime._historical_session_row(session_id)
        ...
        runtime._record_metric("api_messages_init_ms" if init else "api_messages_poll_ms", dt_total_ms)
        return True
    ...
    if path.startswith("/api/sessions/") and path.endswith("/tail"):
        ...
        try:
            ...
            runtime._json_response(handler, 404, {"error": "unknown session"})
            return True
        runtime._json_response(handler, 200, {"tail": tail})
        return True
