...
from codoxear.sessions import state_machine as _session_state_machine
...
def _strip_ansi(text: str) -> str:
    return _session_state_machine.strip_ansi(text)
...
def _update_busy_from_pty_text(st: "State", text: str, now_ts: float) -> None:
    _session_state_machine.update_busy_from_pty_text(st, text, now_ts=now_ts)


def _should_clear_busy_state(st: "State", now_ts: float) -> bool:
    return _session_state_machine.should_clear_busy_state(
        st,
        now_ts,
        quiet_seconds=BUSY_QUIET_SECONDS,
        interrupt_grace_seconds=BUSY_INTERRUPT_GRACE_SECONDS,
    )
...
def _apply_rollout_obj_to_state(
    ...
    _session_state_machine.apply_rollout_obj_to_state(st, obj, now_ts=now_ts)
