...
class TestBackendSeamsSource(unittest.TestCase):
    ...
    def test_server_delegates_session_control_send_and_queue_flows(self) -> None:
        ...
        self.assertIn("return _session_creation.parse_create_session_request(obj)", source)
        self.assertIn("return _session_creation.read_codex_launch_defaults()", source)
        ...
        self.assertIn("return _session_listing.normalize_session_cwd_row(row)", source)
        self.assertIn("return _session_listing.frontend_session_list_row(row)", source)
        self.assertIn("return _session_listing.session_list_group_key(row)", source)
        ...
        self.assertIn("return _workspace_file_access.read_client_file_view(path_obj)", source)
        ...
        self.assertIn("return _resume_candidates.iter_all_resume_candidates(limit=limit)", source)
        self.assertIn("return _resume_candidates.pi_resume_candidate_from_session_file(session_path)", source)
        self.assertIn("return _pi_session_files.pi_new_session_file_for_cwd(cwd)", source)
        ...
        self.assertIn("return _pi_session_files.copy_file_atomic(source_path, target_path)", source)
